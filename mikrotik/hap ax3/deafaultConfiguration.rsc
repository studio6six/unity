...
:local ifcId [/interface wifi find where default-name=wifi2]
 ...
 /system routerboard wps-button set enabled=yes
 /system routerboard wps-button set on-event=wps-accept
 /system script add name=wps-accept comment="defconf" source={
...
:if ($action = "revert") do={
 ...
 /system routerboard wps-button set on-event=""
 /system script remove [find comment~"defconf"]
 /disk settings
